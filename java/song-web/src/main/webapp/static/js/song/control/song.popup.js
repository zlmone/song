...
;(function (song, j) {
    ...
    song.extend(song.popup, song.mask, {
        ...
        position: function (p) {
            this.wrap.css(p || (this.el ? song.position.getOffsetSize(j(this.el)) : song.position.maxClient()));
            return this;
        }
    });
    /************************loading************************/
    ...
    song.extend(song.popup, song.tip, {
        ...
        load: function (url, dataType, fn) {
            if (!this.theRequest) {
                ...
                j.ajax({
                    type: "get", dataType: dataType, url: url, success: function (data) {
                        me.theRequest = true;
                        if (dataType == "text") {
                            me.setContent(data);
                        } else {
        ...
        close: function () {
            if (this.onClose) if (this.onClose.call(this) == false) {
                return;
            }
            ;
            this.clearTimer();
